...
function buildTrack() {
  ...
  if (TRACK_POINTS && TRACK_POINTS.length > 0) {
    // Use custom drawn points
    TRACK_POINTS.forEach(p => globalPts.push({ x: p.x, y: p.y }));
  } else {
    // Fallback to asset-based points
    TRACK_DATA.forEach((block) => {
      const d = BLOCK_DEFS[block.type];
      const localPts = d.p(d.w, d.h);
      localPts.forEach(lp => {
        globalPts.push({ x: block.x + lp[0] * BLOCK_SCALE, y: block.y + lp[1] * BLOCK_SCALE });
      });
    });
  }
...
class Bike {
  ...
  update(input) {
    ...
    // ── Engine ──────────────────────────────────────────────
    if (input.fwd) { this._spd = Math.min(this._spd + ACCEL, MAX); }
...
class Environment {
  constructor(terrain) {
    ...
    if (PROPS_DATA && PROPS_DATA.length > 0) {
      ...
      PROPS_DATA.forEach(p => {
        // Map filename to ASSET_MAP key
        let key = Object.keys(ASSET_MAP).find(k => ASSET_MAP[k] === p.asset);
        if (key) {
          this.props.push({
            key, x: p.x, y: p.y,
            scale: p.scale || 1.0,
  ...
  drawProps(ctx, am, cam) {
    const vis = this.props.filter(p => p.x - cam.x < C.W + 600 && p.x - cam.x > -400);
    vis.forEach(p => {
      const img = am.get(p.key);
      if (img) { ctx.drawImage(img, p.x - p.w * .5, p.y - p.h, p.w, p.h); }
      else { ctx.fillStyle = p.isSenaro ? '#e8c44a' : '#558855'; ctx.fillRect(p.x - p.w * .5, p.y - p.h, p.w, p.h); }
      if (p.isSenaro) {
        ctx.save();
        ctx.fillStyle = '#FFD700'; ctx.fillRect(p.x - p.w * .5 - 10, p.y - p.h - 90, 5, 90);
        ctx.fillStyle = '#FF4500'; ctx.fillRect(p.x - p.w * .5 - 5, p.y - p.h - 90, 32, 22);
        ctx.restore();
      }
    });
  }
...
function drawHUD(ctx, dist, elapsed, speed, vW, vH) {
  ...
  ctx.fillText(`📍 ${Math.max(0, dist) | 0} m`, pad + 16, pad + 29);
...
class MenuScene {
  ...
  draw(ctx, vW, vH) {
    ...
    ctx.save(); ctx.translate(sx, sy); ctx.rotate(this.t * .3);
    for (let i = 0; i < 12; i++) {
      ctx.fillStyle = 'rgba(255,200,0,.14)'; ctx.beginPath(); ctx.moveTo(0, 0);
      ctx.arc(0, 0, sr + 45, i * Math.PI / 6, i * Math.PI / 6 + .22); ctx.closePath(); ctx.fill();
    }
    ctx.restore();
  ...
  _btn(ctx, x, y, label, id) {
    const w = 270, h = 52, bx = x - w / 2, by = y - h / 2, hot = this.hover === id;
    ctx.save();
    ctx.fillStyle = hot ? 'rgba(255,140,0,.88)' : 'rgba(255,100,0,.55)';
    ctx.beginPath(); ctx.roundRect(bx, by, w, h, 26); ctx.fill();
    ctx.strokeStyle = hot ? '#FFD700' : 'rgba(255,215,0,.5)'; ctx.lineWidth = hot ? 3 : 2;
    ctx.beginPath(); ctx.roundRect(bx, by, w, h, 26); ctx.stroke();
    ctx.font = '700 21px Outfit'; ctx.textAlign = 'center'; ctx.fillStyle = '#fff';
    ctx.fillText(label, x, y + 7); ctx.restore();
    this._btns[id] = { bx, by, w, h };
  }
  click(mx, my) {
    for (const [id, r] of Object.entries(this._btns)) {
      if (mx >= r.bx && mx <= r.bx + r.w && my >= r.by && my <= r.by + r.h) {
        if (id === 'start') this.game.switchScene('game');
        else this.hover = this.hover === 'help' ? null : 'help'; return;
      }
    }
  }
  move(mx, my) {
